...
const SuratKeluarPage = () => {
  ...
  useEffect(() => {
    // TODO: Fetch surat keluar data from API
    // This is just mock data for now
    setSuratKeluar([
      {
        id: 1,
        tertuju: 'Kepala Dinas',
        namasuratklr: 'Surat Undangan',
        perihal: 'Undangan Rapat',
        nosurat: '001/SK/2024',
        status: 0,
        komen: 'Menunggu validasi'
      },
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 1:
        return <StatusBadge className="success">Diterima</StatusBadge>;
      case 2:
        ...
        <Table>
          ...
          <tbody>
            {suratKeluar.map((surat) => (
              <tr key={surat.id}>
                <td>{surat.id}</td>
                <td><b>{surat.tertuju}</b></td>
                <td>{surat.namasuratklr}</td>
                <td>{surat.perihal}</td>
                <td>{surat.nosurat}</td>
                ...
                <td>
                  ...
                  {authUser?.role === 'Karo' && (
                    <ActionButton className="warning" onClick={() => window.location.href = `/validasi/${surat.id}`}>
                      <FaCheck /> Validasi
                    </ActionButton>
                  )}
                </td>
