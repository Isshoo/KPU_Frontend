...
const SuratMasukPage = () => {
  ...
  const fetchSuratMasuk = async (page = 1) => {
    try {
      setLoading(true);
      const response = await _fetchWithAuth(`${BASE_URL}/surat-masuk/?page=${page}`);
      if (!response.ok) {
        throw new Error('Failed to fetch surat masuk');
      }
      const data = await response.json();
      console.log(data);
      setSuratMasuk(data.surat_list);
  ...
  const handleDownload = async (filePath) => {
    try {
      const response = await _fetchWithAuth(`${BASE_URL}/surat-masuk/download/${filePath}`);
      if (!response.ok) {
